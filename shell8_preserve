...
- This page is sovereign: no other page may overwrite its state.
- Change FILE_ID / ROOM_ID / VERSION ONLY when you intentionally branch.
========================================================= */
const FILE_ID = "KETADATA_SHELL_V4";
const ROOM_ID = "BASE";
const VERSION_ID = "shellv4.base.v3";
const STORAGE_KEY = `KDT::STATE::${FILE_ID}::${ROOM_ID}::${VERSION_ID}`;

// WB ▸ GLOBAL NOTE KEY (INTENTIONAL GLOBAL)
// - Page state is sovereign (STORAGE_KEY).
// - KETA_NOTE text is continuous across pages via this key.
...
const DEFAULT = {
  ...
  // Minimal: roomId + stateId, plus optional categoryIds.
  ketaNoteMeta: {
...
// If a global note exists, it overrides page-local note text.
...
- No UI changes.
- Meta is kept in STATE.ketaNoteMeta and exported with STATE.
========================================================= */
...
function syncKetaNoteMeta(reason){
  ...
  // room labeling: canonicalized from STATE.room
  const room = (STATE.room || "BASE").trim();
  const roomId = room.toUpperCase();

  // minimal state labeling: derived from stableSig()
  ...
  // (optional) reason is kept in payload for forensic export without UI
  if(reason){
...
function persist(){
  ...
  // WB ▸ keep global note in sync (idempotent)
...
function mergeDefault(obj){
  // WB: tolerant merge (keeps unknown keys if present)
...
function exportState(){
  // ensure export contains up-to-date labels
...
function importState(text){
  ...
  // Accept either {STATE} envelope or raw STATE
  const incoming = parsed && parsed.STATE ? parsed.STATE : parsed;

  STATE = mergeDefault(incoming);
  // WB ▸ GLOBAL NOTE CONTINUITY
  // Imported note becomes the global note (explicit, controlled globalness).
  saveGlobalKetaNote(STATE.ketaNote || "");
  stopLight(true);
  if(STATE.ui.lightRunning) startLight(STATE.ui.lightPreset);

  // align labels immediately after import (room/state)
...
Behavior: space toggles LIGHT and MOTION together (kernel transport).
========================================================= */
...
/* =========================================================
EE ▸ LIGHTS 1–6 (your default strobe engines)
========================================================= */
...
function render(){
  ...
  // room url affordance (only if exists)
...
$("btnSaveRoom").addEventListener("click", ()=>{
  STATE.room = ($("roomInput").value || "BASE").trim();
...
WB: tolerant state merge; optional roomUrl jump; hotkeys; drag; LOCALSTORAGE SOVEREIGNTY
========================================================= */
</script>
</body>
</html>
...
LOCALSTORAGE_KEY: KDT::STATE::KETADATA_SHELL_V4::BASE::shellv4.base.v3
EXPORT_ENVELOPE: { STORAGE_KEY, FILE_ID, ROOM_ID, VERSION_ID, STATE }
============================================================
CHANGELOG:
- 2025-12-23: PAGE SOVEREIGN STORAGE KEY (NO SHARED STATE COLLISIONS)
- 2025-12-23: ROOM DEFAULT SET TO BASE (WAS LOBBY IN LEGACY STATES)
- 2025-12-23: EXPORT ENVELOPE INCLUDES FILE/ROOM/VERSION + STORAGE_KEY
- 2025-12-23: KETA_NOTE TEXT MADE GLOBAL (CONTINUOUS ACROSS SOVEREIGN PAGES)
============================================================
...
- Stores many rooms + urls as presets.
- Active room remains STATE.room / STATE.roomUrl (existing behavior).
- Presets stored in STATE.payload.roomPresets (non-breaking).
========================================================= */
...
- CLICK PRESET: LOAD FIELDS
- DOUBLE CLICK PRESET: SET ACTIVE + PERSIST
============================================================
CHANGELOG:
- 2025-12-23: PAGE SOVEREIGN STORAGE KEY (NO SHARED STATE COLLISIONS)
- 2025-12-23: ROOM DEFAULT SET TO BASE (WAS LOBBY IN LEGACY STATES)
- 2025-12-23: EXPORT ENVELOPE INCLUDES FILE/ROOM/VERSION + STORAGE_KEY
- 2025-12-23: KETA_NOTE TEXT MADE GLOBAL (CONTINUOUS ACROSS SOVEREIGN PAGES)
- 2025-12-24: ROOM PRESETS ADDED (MULTI-ROOM REGISTRY + ACTIVE ROOM)
============================================================
-->
-->
